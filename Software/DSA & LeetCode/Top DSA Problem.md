---
date: 2024-11-19
tags: 
link:
---
# Top 25 Most Frequently Asked DSA Questions in MAANG

1. [Two Sum](https://leetcode.com/problems/two-sum/)
2. [Product of Array Except Self](https://leetcode.com/problems/product-of-array-except-self/)
3. [Median of Two Sorted Arrays](https://leetcode.com/problems/median-of-two-sorted-arrays/)
4. [Search in Rotated Sorted Array](https://leetcode.com/problems/search-in-rotated-sorted-array/)
5. [Maximum Subarray](https://leetcode.com/problems/maximum-subarray/)
6. [Best Time to Buy and Sell Stock](https://leetcode.com/problems/best-time-to-buy-and-sell-stock/)
7. [Trapping Rain Water](https://leetcode.com/problems/trapping-rain-water/)
8. [3Sum](https://leetcode.com/problems/3sum/)
9. [Permutations](https://leetcode.com/problems/permutations/)
10. [Word Search](https://leetcode.com/problems/word-search/)
11. [Longest Substring Without Repeating Characters](https://leetcode.com/problems/longest-substring-without-repeating-characters/)
12. [Longest Common Prefix](https://leetcode.com/problems/longest-common-prefix/)
13. [Longest Palindromic Substring](https://leetcode.com/problems/longest-palindromic-substring/)
14. [Valid Parentheses](https://leetcode.com/problems/valid-parentheses/)
15. [Reverse Linked List](https://leetcode.com/problems/reverse-linked-list/)
16. [Linked List Cycle](https://leetcode.com/problems/linked-list-cycle/)
17. [Add Two Numbers](https://leetcode.com/problems/add-two-numbers/)
18. [Merge k Sorted Lists](https://leetcode.com/problems/merge-k-sorted-lists/)
19. [Top K Frequent Elements](https://leetcode.com/problems/top-k-frequent-elements/)
20. [Course Schedule](https://leetcode.com/problems/course-schedule/)
21. [Number of Islands](https://leetcode.com/problems/number-of-islands/)
22. [Non-overlapping Intervals](https://leetcode.com/problems/non-overlapping-intervals/)
23. [Meeting Rooms II](https://leetcode.com/problems/meeting-rooms-ii/)
24. [LRU Cache](https://leetcode.com/problems/lru-cache/)


## Subarrays related sum

1. [maximum-sum-of-distinct-subarrays-with-length-k](https://leetcode.com/problems/maximum-sum-of-distinct-subarrays-with-length-k)
2. [maximum-erasure-value](https://leetcode.com/problems/maximum-erasure-value)
3. [defuse-the-bomb](https://leetcode.com/problems/defuse-the-bomb)

## Subset

1. [word-subsets](https://leetcode.com/problems/word-subsets/)
## Sliding window

1. [best-sightseeing-pair](https://leetcode.com/problems/best-sightseeing-pair)
## Binary Search

1. [minimum-limit-of-balls-in-a-bag](https://leetcode.com/problems/minimum-limit-of-balls-in-a-bag)
2. [Koko eating bananas](https://leetcode.com/problems/koko-eating-bananas/)
3. [find-longest-special-substring-that-occurs-thrice-i](https://leetcode.com/problems/find-longest-special-substring-that-occurs-thrice-i)
4. [longest-increasing-subsequence](https://leetcode.com/problems/longest-increasing-subsequence) - solve through binary search and dynamic programming
## Dynamic Programming (BFS and DFS)

1. [Coin Change](https://leetcode.com/problems/coin-change)
2. [minimum-cost-for-tickets](https://leetcode.com/problems/minimum-cost-for-tickets)
## Back tracking & Recursion

1. [letter-combinations-of-a-phone-number](https://leetcode.com/problems/letter-combinations-of-a-phone-number)
2. [generate-parentheses](https://leetcode.com/problems/generate-parentheses/)
3. [subsets](https://leetcode.com/problems/subsets/)
## Two pointer

1. [move-pieces-to-obtain-a-string](https://leetcode.com/problems/move-pieces-to-obtain-a-string)
2. [Sort colors](https://leetcode.com/problems/sort-colors)
3. [next-permutation](https://leetcode.com/problems/next-permutation)
## Matrix & Grid

1. [rotating-the-box](https://leetcode.com/problems/rotating-the-box)
2. [first-completely-painted-row-or-column](https://leetcode.com/problems/first-completely-painted-row-or-column)
3. [Grid game](https://leetcode.com/problems/grid-game/)
4. [rotating image](https://leetcode.com/problems/rotate-image/)

## Tree Node

1. [reverse-odd-levels-of-binary-tree](https://leetcode.com/problems/reverse-odd-levels-of-binary-tree)
## Maths

1. [unique-paths](https://leetcode.com/problems/unique-paths/description/)  and [unique-paths-II](https://leetcode.com/problems/unique-paths-ii)